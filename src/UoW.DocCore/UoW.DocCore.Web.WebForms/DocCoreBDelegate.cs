...
namespace UoW.DocCore.Web.WebForms
{
    public sealed class DocCoreBDelegate
    {
        ...
        public int UpdateUserAccess(UserDto newUser)
        {
            int status = -1;
            ...
            if (newUser != null)
            {
                ...
                //Add New user
                string val = RestClient.Instance.MakeHttpRequest(Service_BaseAddress + "/userRest/UpdateUserAccess", "POST", json_type, userjson);
                status = val != null ? Int32.Parse(val) : -1;
            }
            return status;
        }
